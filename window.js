...
function updateLocationDisplay() {
  ...
  // 1. Prioritas Utama: Input Manual (Custom Location)
  if (state.customLocation && state.customLocation.trim() !== "") {
    elements.locationDisplay.textContent = `LOC: ${state.customLocation}`;
    return;
  }

  // Tampilkan efek loading sementara saat meminta izin GPS
  // --- PINTU GERBANG PRIVASI ---
  // Jika sakelar dimatikan, tulis Nonaktif dan hentikan semua fungsi pelacakan!
  if (!state.locationEnabled) {
    elements.locationDisplay.textContent = "LOC: Nonaktif";
    return; // Perintah 'return' ini akan membatalkan semua pencarian GPS/IP di bawahnya
  }

  // ... (Sisa kode pelacakan lokasi yang sudah kamu miliki sebelumnya tetap di sini) ...
  elements.locationDisplay.textContent = "LOC: Mendeteksi...";

  // --- Fungsi Jaring Pengaman (Fallback ke Timezone) ---
  const useFallback = () => {
    ...
    try {
      ...
      console.error("Gagal deteksi area timezone", e);
    }
    elements.locationDisplay.textContent = `LOC: ${areaName}`;
  };

  // 2. Coba Lacak GPS Akurat
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          // Terjemahkan Titik Kordinat jadi Nama Kota pakai OpenStreetMap API (Gratis & Aman)
          ...
          // Ambil nama kota/kabupaten/kecamatan yang paling akurat
          const exactLocation =
            data.address.city ||
            data.address.town ||
          ...
          // Kalau API petanya lagi down, kembali ke Timezone
          console.warn("Gagal konversi nama kota:", err);
          useFallback();
        }
      },
      (error) => {
        // Jika user klik "Block" di pop-up izin, langsung mundur ke Timezone
        console.warn("GPS Ditolak/Timeout:", error.message);
        useFallback();
      },
      { timeout: 5000, enableHighAccuracy: false }, // Jangan tunggu kelamaan (max 5 detik)
    );
  } else {
    // Kalau browsernya sangat jadul dan tidak ada fitur GPS
    useFallback();
  }
}
...
function updateClock() {
  const now = new Date();
  elements.pcTime.textContent = formatJustTime(now);
  elements.pcDate.textContent = formatJustDate(now);
...
async function syncNow(silent = false) {
  ...
  try {
    ...
    if (!silent) {
      ...
      elements.syncBtnText.textContent = "FORCE SYNC";
    }
  }
}
...
elements.settingsBtn.addEventListener("click", () => {
  ...
  // Tidak ada lagi perintah resize jendela di sini
});
...
elements.locationInput.addEventListener("input", (e) => {
  // 1. Sterilkan inputan user secara real-time
  const safeValue = sanitizeInput(e.target.value);

  // 2. Jika user mencoba mengetik karakter terlarang (< atau >), paksa hapus dari kotak input
...
document.querySelectorAll(".quick-select-btn").forEach((btn) => {
  btn.addEventListener("click", (e) => {
    state.ntpServer = e.currentTarget.getAttribute("data-server");
    elements.ntpInput.value = state.ntpServer;
    saveSettings();
...
loadSettings();
initTheme();
updateLocationDisplay(); // <-- TAMBAHKAN BARIS INI
